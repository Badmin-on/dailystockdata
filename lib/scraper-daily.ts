...
// 최신 주가 1개만 수집 (원래 코드 방식 - 빠름!)
export async function fetchLatestStockPrice(stockCode: string) {
  ...
  try {
    ...
    // 전일대비 (등락) - cells[2]에서 가격 차이 추출
    const priceChangeText = $(cells[2]).text().trim();
    const isDown = priceChangeText.includes('▼');
    const priceChange = cleanNumber(priceChangeText);
    
    // 변동률 (%) - cells[4]에 있음! (cells[3]은 대비, cells[4]가 등락률)
    const changeRateText = $(cells[4]).text().trim();
    const changeRateRaw = cleanNumber(changeRateText);
    const changeRate = isDown && changeRateRaw ? -changeRateRaw : changeRateRaw;

    // 거래량
    const volume = cleanNumber($(cells[6]).text().trim());

    if (closePrice === null || !date) return null;
    ...
    console.error(`Error fetching latest price for ${stockCode}:`, error.message);
    return null;
  }
}
