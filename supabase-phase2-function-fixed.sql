-- ============================================
-- Phase 2 최적화: get_date_comparison() Function (NUMERIC 캐스팅 수정)
-- 버그 수정: 정수 나누기 → NUMERIC 나누기로 정확한 growth_rate 계산
-- 수정 내용: ::NUMERIC 캐스팅 추가
-- ============================================

-- 기존 Function 삭제
DROP FUNCTION IF EXISTS get_date_comparison(TEXT, TEXT, TEXT, INT, NUMERIC, TEXT, INT);

-- 수정된 Function 생성 (정확한 계산)
  ...
  comparison AS (
    SELECT
      ...
      CASE
        ...
        -- 양수에서 시작 (NUMERIC 캐스팅으로 정확한 계산)
        WHEN s.metric_value > 0 THEN
          ((e.metric_value - s.metric_value)::NUMERIC / s.metric_value::NUMERIC) * 100
        -- 음수에서 시작 (NUMERIC 캐스팅으로 정확한 계산)
        WHEN s.metric_value < 0 THEN
          ((e.metric_value - s.metric_value)::NUMERIC / ABS(s.metric_value)::NUMERIC) * 100
        ELSE NULL
      END as growth_rate,
