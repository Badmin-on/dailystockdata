-- Safe Update for v_investment_opportunities View
-- 실행 방법: Supabase SQL Editor에서 복사 후 실행
-- 목적: 기존 View를 안전하게 동적 년도 필터로 업데이트

-- Step 1: 현재 View 정의 확인 (참고용)
-- SELECT definition FROM pg_views WHERE viewname = 'v_investment_opportunities';

-- Step 2: 기존 View 삭제 (CASCADE로 의존성도 함께 처리)
DROP VIEW IF EXISTS v_investment_opportunities CASCADE;

-- Step 3: 동적 년도 필터가 적용된 View 재생성
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '테스트 쿼리: SELECT COUNT(*) FROM v_investment_opportunities;';
END $$;
