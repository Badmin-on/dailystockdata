-- YoonStock Pro - Enhanced Schema for Consensus Tracking & Investment Scoring
-- 실행 방법: Supabase SQL Editor에서 실행

-- ============================================
-- 1. 120일 이동평균 및 이격도 계산 함수
-- ============================================

-- 120일 이동평균 계산 함수
CREATE OR REPLACE FUNCTION calculate_ma_120(p_company_id INT, p_date DATE)
...
-- 이격도 계산 함수
CREATE OR REPLACE FUNCTION calculate_divergence(
...
-- ============================================
-- 2. 재무 컨센서스 변화율 Materialized View
-- ============================================

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_consensus_changes AS
WITH latest_date AS (
...
CREATE INDEX IF NOT EXISTS idx_mv_consensus_changes
  ON mv_consensus_changes(revenue_change_1m DESC, op_profit_change_1m DESC);

-- ============================================
-- 3. 주가 분석 Materialized View (120일 이평선 + 이격도)
-- ============================================

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_stock_analysis AS
...
ma_calculations AS (
  SELECT
    ...
    lp.change_rate,
    lp.volume,
    calculate_ma_120(lp.company_id, lp.latest_date) as ma_120
...
SELECT
  ...
  mc.volume,
  mc.ma_120,
  calculate_divergence(mc.current_price, mc.ma_120) as divergence_120,
  w52.week_52_high,
  w52.week_52_low,
  CASE
...
CREATE INDEX IF NOT EXISTS idx_mv_stock_divergence
  ON mv_stock_analysis(divergence_120);

-- ============================================
-- 4. 투자 점수 계산 View (컨센서스 + 이격도)
-- ============================================
...
-- Supabase에서는 pg_cron을 사용하거나 Vercel Cron으로 API 호출
-- 예시: 매일 오후 9시 자동 갱신
-- SELECT cron.schedule('refresh-views', '0 21 * * *', 'SELECT refresh_all_views()');
