-- Update v_investment_opportunities View with 4-Year Dynamic Filter
-- 실제 Supabase View 구조에 맞춘 업데이트
-- 목적: Year-1, Year, Year+1, Year+2 (총 4년)만 표시
-- 실행 방법: Supabase SQL Editor에서 전체 복사 후 실행

-- 기존 View 삭제
DROP VIEW IF EXISTS v_investment_opportunities CASCADE;

-- 4년 동적 필터가 적용된 View 재생성
...
WITH consensus_scores AS (
    ...
    -- 🔥 4년 동적 필터: Year-1부터 Year+2까지 (총 4년)
    WHERE year BETWEEN
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    RAISE NOTICE '📅 4년 동적 필터 적용: Year-1 ~ Year+2';
    RAISE NOTICE '🔄 매년 1월 1일 0시 자동으로 필터 업데이트';
    RAISE NOTICE '';
    RAISE NOTICE '현재 년도: %', current_yr;
    RAISE NOTICE '필터 범위: % ~ % (총 4년)', current_yr - 1, current_yr + 2;
    RAISE NOTICE '';
    RAISE NOTICE '년도별 예시:';
    RAISE NOTICE '  - 2025년: 2024, 2025, 2026, 2027 표시';
    RAISE NOTICE '  - 2026년: 2025, 2026, 2027, 2028 표시';
    RAISE NOTICE '  - 2027년: 2026, 2027, 2028, 2029 표시';
    RAISE NOTICE '';
    RAISE NOTICE '변경 사항:';
    RAISE NOTICE '  - BEFORE: 모든 년도 표시 (2024~2028, 총 5년)';
    RAISE NOTICE '  - AFTER: % ~ % 만 표시 (총 4년)', current_yr - 1, current_yr + 2;
    RAISE NOTICE '';
    RAISE NOTICE '검증 쿼리:';
